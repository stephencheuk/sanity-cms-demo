        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col p-5 my-10 max-w-2xl mx-auto mb-10">
          ...
          <label className="block mb-5">
            ...
            <input
              ...
              className="shadow border rounded py-2 px-3 form-input mt-1 block w-full ring-yellow-500 outline-none focus:ring" placeholder="your@email.com" type="Email" />
          </label>
          <label className="block mb-5">
            <span className="text-gray-700">Comment</span>
            <textarea
              { ...register("comment", { required: true }) }
              className="shadow border rounded py-2 px-3 form-textarea mt-1 block w-full ring-yellow-500 outline-none focus:ring" placeholder="Input some comment" rows={8}></textarea>
          </label>
...
export const getStaticPaths = async () => {
  ...
  const paths = posts.map((post: Post) => ({
    params: {
      slug: post.slug.current, /* echo to the file name */
    }
  }));
